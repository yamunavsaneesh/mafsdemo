...
class MAFS_Controller extends CI_Controller
{
    ...
    public function fronthead()
    {
        ...
        endforeach;
        $scats = $this->Servicecategory_model->get_array();
    ...
    public function frontleftmenu($datas)
    {
        $left['datas'] = $datas;
        return $this->load->view('frontend/include/left', $left, true);
    }
    public function frontprintcontent($frontcontents, $leftboxoption = true, $products = true)
    {
        if ($leftboxoption) {
    ...
    public function frontfooter($latestnews = false)
    {
        $this->load->model('frontend/contacts_model');
        $footermenus = array();
        $footer['contact'] = $this->contacts_model->get_row_cond(array('category_id' => 1));
        $footer['footer'] = $this->menuitems_model->get_menu('footer');
        $footer['policy'] = $this->menuitems_model->get_policy_menu('policy');
    ...
    public function sendfromadmin($to, $subject, $message, $attachment = '')
    {
        $this->load->library('email');
        $config['mailtype'] = 'html';
        $config['wordwrap'] = true;
    ...
    public function sendtoadmin($fromemail, $fromname, $attachment, $subject, $message)
    {
        $this->load->library('email');
        $config['mailtype'] = 'html';
        $config['wordwrap'] = true;
    ...
    public function adminnotification($subject, $message)
    {
        $this->load->library('email');
        $config['mailtype'] = 'html';
        $config['wordwrap'] = true;
    ...
    public function adminnotification2($subject, $message, $tomail)
    {
        $this->load->library('email');
        $config['mailtype'] = 'html';
        $config['wordwrap'] = true;
    ...
    public function adminnotification3($subject, $message, $tomail)
    {
        $this->load->library('email');
        $config['mailtype'] = 'html';
        $config['wordwrap'] = true;
    ...
    public function sendtofriend($fromemail, $fromname, $toemail, $toname, $subject, $message)
    {
        $this->load->library('email');
        $config['mailtype'] = 'html';
