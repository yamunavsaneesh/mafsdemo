...
<div class="full_w">
    ...
    <?php echo form_open('admin/video/actions/' . $this->uri->segment(4)); ?>
    <div class="entry">
        ...
        <div style="float:right;"> Sort :
            ...
            <select name="category" style=" margin-right:5px;">
                ...
                <option value="<?php echo $gallerycat['id']; ?>"
                    <?php if ($this->session->userdata('gallery_category_id') == $gallerycat['id']) {echo 'selected="selected"';}?>>
                    <?php echo $gallerycat['title']; ?></option>
                <?php endforeach;?>
            </select>
            Search :
            <input style="margin-right:5px;" type="text" name="keyword"
                value="<?php echo $this->session->userdata('gallery_key'); ?>" />
            <select name="field" style="margin-right:5px;">
                <?php foreach ($galleryfields as $id => $key): ?>
                <option value="<?php echo $id; ?>"
                    <?php if ($this->session->userdata('gallery_field') == $id) {echo 'selected="selected"';}?>>
                    <?php echo $key; ?></option>
                <?php endforeach;?>
            </select>
...
foreach ($galleries as $gallery): ?>
            <tr>
                <td class="align-center"><input type="checkbox" name="id[]" class="chkbx"
                        value="<?php echo $gallery['id']; ?>" /></td>
                <td class="align-center"><?php echo ++$i; ?></td>
                <?php $youtubeID = getYouTubeVideoId($gallery['video']);
$thumbURL = 'https://img.youtube.com/vi/' . $youtubeID . '/default.jpg';?>
                ...
                <td align="center"><input style="text-align:center;" type="text" size="2"
                        name="sort_order[<?php echo $gallery['id']; ?>]"
                        value="<?php echo $gallery['sort_order']; ?>" /></td>
                <td align="center"><?php echo $activearr[$gallery['status']]; ?></td>
                <td><a href="<?php echo site_url('admin/video/edit/' . $this->uri->segment(4) . '/' . $gallery['id'] . '/' . $return); ?>"
                        class="table-icon edit" title="Edit"></a> <a
                        href="<?php echo site_url('admin/video/delete/' . $gallery['id'] . '/' . $return); ?>"
    ...
    <div class="entry">
        ...
        <div class="sep"></div>
        <a class="button add" href="<?php echo site_url('admin/video/add/' . $this->uri->segment(4)); ?>">Add New Video
            Gallery</a>
    </div>
</div>