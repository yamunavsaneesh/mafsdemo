<div class="full_w"> 
	<div class="h_title">Add New Setting</div>	 
	<?php echo form_open_multipart('admin/home/addsettings'); ?> 
		...
		<div class="element"> 
			<label for="settingkey">Key <?php if(form_error('settingkey')){ $err=' err'; echo form_error('settingkey'); } else { $err=''; ?><span> (required)</span><?php } ?></label> 
			<input id="settingkey" name="settingkey" type="text" class="text<?php echo $err; ?>" value="<?php echo set_value('settingkey'); ?>" /> 
		</div> 
        <div class="element"> 
			<label for="settingtype">Type<?php if(form_error('settingtype')){ $err=' err'; echo form_error('settingtype'); } else { $err=''; ?><span> (required)</span><?php } ?></label> 
			<input id="settingtype" name="settingtype" type="text" class="text<?php echo $err; ?>" value="<?php echo set_value('settingtype'); ?>" /> 
		</div> 
		<div class="element"> 
			<label for="settingvalue">Value (<?php echo $this->languagesarr[$this->session->userdata('admin_language')]?>)<?php if(form_error('settingvalue')){ $err=' err'; echo form_error('settingvalue'); } else { $err=''; ?><span> (required)</span><?php } ?></label> 
		...
		<div class="entry"> 
			<button type="submit" class="add">Save</button><a class="button cancel" href="<?php echo site_url('admin/home/addsettings'); ?>">Cancel</a> 
		</div> 
	<?php echo form_close(); ?> 
</div>